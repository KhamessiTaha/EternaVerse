...
export class UniverseGateway {
  ...
  handleJoinUniverse(@MessageBody() universeId: string, @ConnectedSocket() client: Socket) {
    client.join(`universe-${universeId}`);
    return { message: `Joined universe ${universeId}` };
  }

  @SubscribeMessage('leaveUniverse')
  handleLeaveUniverse(@MessageBody() universeId: string, @ConnectedSocket() client: Socket) {
